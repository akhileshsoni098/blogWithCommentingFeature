...
exports.createAdmin = async function (req, res) {
    try {
        ...
        if (Object.keys(authorData).length != 0) {
            ...
            const emailExist = await adminModel.findOne({ email: authorData.email })
            if (emailExist) {
            ...
            const saveData = await adminModel.create(authorData);
...
exports.logInAdmin = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length != 0) {
            let author = await adminModel.findOne({ email: email, password: password });
            if (!author) {
...
//============================= genrating token with some credantial by secrete-key ===================================
            ...
            const token = jwt.sign({ _id: author._id.toString() }, process.env.JWT_SECRET_KEY);
